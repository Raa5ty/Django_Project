...
from tgservice.prompts import system_keywords, user_keywords
...
class Command(BaseCommand):
    help = 'Выявляет ключевые слова для каналов через OpenAI Batch API'
    ...
    def handle(self, *args, **options):
        ...
        if input_file is None:
            print("✅ Все каналы уже получили ключевые слова.")
            return
    ...
    def create_jsonl(self):
        ...
        system_prompt = system_keywords  # Мы всегда работаем с профилем
        user_prompt = user_keywords

        # Делаем запрос к базе данных с помощью Django ORM
        worksheets = WorkSheet.objects.filter(keywords_channel=None)
        ...
        # Путь к файлу
        file_path = os.path.join(self.results_dir, "get_keywords.jsonl")
    ...
    def process_batch_split(self, input_file, max_lines=350, endpoint="/v1/chat/completions", completion_window="24h"):
        ...
        for idx, part in enumerate(parts):
            ...
            if batch_job.status in ["completed", "expired"]:
                ...
                # Техническая пауза между обработкой частей файла
                if idx < len(parts) - 1:
                    print("Техническая пауза перед следующей частью 30 секунд...")
                    time.sleep(30)
    ...
    def update_db(self, result_file):
        ...
        for res in results:
            ...
            if record:
                ...
                record.keywords_channel = content
                record.keywords_intokens = input_tokens
                record.keywords_outtokens = output_tokens
                record.save()  # Сохраняем изменения в базе
            else:
                print(f"Запись с channel_name '{channel_name}' не найдена. Пропускаем.")

        self.stdout.write(self.style.SUCCESS("Keywords данные для ТГ-каналов успешно обновлены."))